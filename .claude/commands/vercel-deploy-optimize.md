...
- --analyze: Bundle analysis (precisa setup)
- --preview: Deploy preview

Ex: `vercel-deploy-optimize production --analyze`
...
- Build output (if build exists): !`ls -la .next/ 2>/dev/null || echo "No build found"`
...
```json
{
  ...
  ], // Veja https://vercel.com/docs/cron-jobs para mais exemplos
  "headers": [
    {
      "source": "/api/(.*)",
      "headers": [
...
```bash
**Setup bundle analyzer primeiro (npm i -D @next/bundle-analyzer + config next.config.js)**

Cross-platform: `cross-env ANALYZE=true npm run build`
...
1. Selecione scope (Production/Preview/Development)
2. Add Name/Value (mask secrets)
3. Redeploy
...
vercel env pull .env.local preview
```

Sync envs, verify `vercel env ls production`.
...
#### Preview Deployment
```bash
# Deploy preview from current branch (auto-generates project-abc123.vercel.app)
vercel

# Set custom alias/domain
vercel alias set [deployment-url] preview.yourdomain.com
```
...
# Rollback (ID ou URL completa):
```bash
vercel rollback abc123def456  # Deployment ID (vercel list)
vercel rollback https://project-abc123.vercel.app  # URL
```
...
- [ ] All tests pass (if available)
- [ ] Core Web Vitals scores are optimal (LCP < 2.5s, INP < 200ms, CLS < 0.1)
